#ifndef coinSheet_include_file
#define coinSheet_include_file

#ifdef __cplusplus
extern "C" {
#endif

extern unsigned char coinSheet_tile_0_compressed[131];
extern unsigned char coinSheet_tile_1_compressed[118];
extern unsigned char coinSheet_tile_2_compressed[48];
extern unsigned char coinSheet_tile_3_compressed[108];
#define coinSheet_num_tiles 4
extern unsigned char *coinSheet_tiles_compressed[4];

#ifdef __cplusplus
}
#endif

#endif
